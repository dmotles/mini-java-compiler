...
public class NameVisitor implements Visitor {
    ...
    public void visit(MethodDecl n) {
        ...
        IdentifierType thistype = new IdentifierType( symbolTable.getScopeName().s );
        Identifier thisid = new Identifier("this");

        VariableSymbol thisVar = new VariableSymbol(thistype,thisid);
        ...
        ms.addParameter(thisVar);
        for ( int i = 0; i < n.fl.size(); i++ ) {
            n.fl.elementAt(i).accept(this);
            Formal f = (Formal)n.fl.elementAt(i);
            Type ft = f.t;
            Identifier fi = f.i;
            VariableSymbol fv = new VariableSymbol( ft, fi );
            ms.addParameter(fv);
        }
        for ( int i = 0; i < n.vl.size(); i++ ) {
